...
import { MAZE_GRID_SIZE } from "../game/maze";
import { useMazeStore, MAZE_START, MAZE_GOAL, MAZE_ANIM_DURATION_MS } from "../game/mazeStore";

const TILE = 2;
const HALF = ((MAZE_GRID_SIZE - 1) * TILE) / 2;
...
function worldPos(r: number, c: number): [number, number, number] {
  return [c * TILE - HALF, 0, r * TILE - HALF];
}

const MAX_H = heuristicDistance("euclidean", [0, 0], [MAZE_GRID_SIZE - 1, MAZE_GRID_SIZE - 1]);
...
function MazeTile({ r, c }: TileProps) {
  ...
  const heat = useMemo(() => 1 - Math.min(heuristicDistance(heuristic, [r, c], MAZE_GOAL) / MAX_H, 1), [heuristic, r, c]);
...
function MazeWall({ r, c }: TileProps) {
  const pos = worldPos(r, c);
  return (
...
function Beacon({ cell, color }: { cell: [number, number]; color: THREE.Color }) {
  const lightRef = useRef<THREE.PointLight>(null);
  useFrame(({ clock }) => {
    if (lightRef.current) lightRef.current.intensity = 1.6 + Math.sin(clock.elapsedTime * 2.4) * 0.5;
  });
  const pos = worldPos(cell[0], cell[1]);
  return (
...
export function MazeScene() {
  ...
  const cells = useMemo(() => {
    const walls: TileProps[] = [];
    const floors: TileProps[] = [];
  ...
  return (
    <>
      ...
      <fog attach="fog" args={["#07070a", 26, 60]} />
      <ambientLight intensity={0.18} color="#4a3a5a" />
      <directionalLight position={[10, 20, 10]} intensity={0.25} />
      <Stars radius={80} depth={50} count={2400} factor={2.2} saturation={0} fade speed={0.3} />

      <MazeCamera />

      {cells.floors.map(({ r, c }) => (
        <MazeTile key={`f-${r}-${c}`} r={r} c={c} />
      ))}
      {cells.walls.map(({ r, c }) => (
        <MazeWall key={`w-${r}-${c}`} r={r} c={c} />
      ))}

      <Beacon cell={MAZE_START} color={TEAL} />
      <Beacon cell={MAZE_GOAL} color={AMBER} />
      <PathRunner />
    </>
  );
}
